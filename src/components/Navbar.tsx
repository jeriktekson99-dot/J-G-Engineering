...
interface NavbarProps {
  currentView: 'home' | 'about' | 'services' | 'portfolio' | 'blog' | 'get-started' | 'privacy-policy' | 'terms-of-use' | 'safety-compliance';
...
export default function Navbar({ currentView, setView, onScrollToSection }: NavbarProps) {
  ...
  const handleViewClick = (viewName: 'home' | 'about' | 'services' | 'portfolio' | 'blog' | 'get-started' | 'privacy-policy' | 'terms-of-use' | 'safety-compliance') => {
    setIsOpen(false);
    setView(viewName);
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14">
          ...
          {/* Center/Right: Nav links for wide screens - Exactly: About, Services, Portfolio, Blog */}
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            ...
            <button 
              ...
              className={`font-display font-bold text-sm transition-colors cursor-pointer ${
                currentView === 'blog' ? 'text-industrial-red border-b-2 border-industrial-red pb-1' : 'text-gray-600 hover:text-black hover:border-b hover:border-black pb-1'
              }`}
            >
              Blog
            </button>
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-b border-black">
          <div className="px-2 pt-2 pb-4 space-y-1 sm:px-3 text-left">
            <button
              onClick={() => handleViewClick('about')}
              className="block w-full text-left px-3 py-3 rounded-none text-base font-bold text-gray-700 hover:text-black hover:bg-gray-50 transition-colors"
            >
              About
            </button>
            <button
